...
| [`profile_pt_threaded.jl`](profile_pt_threaded.jl) | Not part of the package — a standalone scaling benchmark for `ThreadedBackend` (prints one CSV line per run: throughput, straggler gap, swap rate). Produced the data in [`docs/pt_profiling_notes.md`](../docs/pt_profiling_notes.md). |
